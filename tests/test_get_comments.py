...
class GetCommentsTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(self.root_comment_count):
            root_comment = CommentMessage.objects.create(
                text="Test text %d" % (i + 1), user=user, parent_id=1,
                parent_type="article")
            self.comment_id_list.add(root_comment.id)
        for i in range(self.comment_depth):
            child_comment = CommentMessage.objects.create(
                text="Child comment %d" % (i + 1), user=user,
                parent_id=root_comment.id, parent_type="comment")
    ...
    def test_receiving_all_comments(self):
        ...
        self.assertEqual(response['count'],
                         self.comment_depth + self.root_comment_count)

    def test_receiving_root_comment(self):
        params = {
            'flat': 1,
            # Means we want to receive only comments for this entity
            # without children
            'parent_type': 'article',
            'parent_id': 1
        }
    ...
    def test_receiving_root_comment_with_children(self):
        params = {
            'flat': 0,
            # Means we want to receive all comments hierarchy starting from
    ...
    def test_pagination(self):
        params = {
            'flat': 0,
            # Means we want to receive all comments hierarchy starting
            # from comments for this entity
            'parent_type': 'article',
            'parent_id': 1
        }
        response = self.client.get('/comments/?%s' % urlencode(params))
    ...
    def test_users_comments_with_error(self):
        while True:
            ...
            if CommentMessage.objects.filter(
                    user=non_existing_user_id).count() == 0:
                break
        response = self.client.get(
            '/comments/users/%d/' % non_existing_user_id)
        self.assertEqual(response.status_code, 200)
        response = json.loads(response.content)
    ...
    def test_speed(self):
        ...
        for i in range(10000):
            CommentMessage.objects.create(text="Test text", user=user,
                                          parent_id=5,
        ...
        params = {
            'flat': 0,
            # Means we want to receive all comments hierarchy
            # starting from comments for this entity
            'parent_type': 'article',
            'parent_id': 5,
            'page_size': 10000
        }
